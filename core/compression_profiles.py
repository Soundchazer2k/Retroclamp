...
class CompressionProfileManager:
    ...
    def _create_default_profiles(self) -> Dict[str, CompressionProfile]:
        ...
        profiles["zstd_balanced"] = CompressionProfile(
            ...
            description=(
                "Balanced compression using ZSTD algorithm (newer CHDMAN versions)"
            ),
        )
    ...
    def get_profile_for_media_type(
        self, media_type: str, level: str = "normal"
    ...
    def get_profiles_for_media_type(self, media_type: str) -> List[CompressionProfile]:
        ...
        media_type = media_type.lower()
        return [
            p for p in self._profiles.values() if p.media_type.lower() == media_type
        ]
    ...
    def detect_console_type(self, file_path: str) -> str:
        ...
        if (
            ext == ".bin"
            or ext == ".cue"
            or (ext == ".iso" and self._get_file_size(file_path) < 700 * 1024 * 1024)
        ):
