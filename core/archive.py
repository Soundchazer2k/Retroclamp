...
class ArchiveWorker(QRunnable):
    ...
    def _extract(self):
        ...
        if os.path.exists(self.output_path):
            ...
            if disk_images:
                print(
                    f"[ArchiveWorker] Skipping extraction: files already present in "
                    f"{self.output_path}."
                )
                ...
                print(
                    "[ArchiveWorker] Emitting finished signal for "
                ...
                print(
                    f"[ArchiveWorker] Extraction skipped: No disk images found in "
                    f"existing folder {self.output_path}."
                )
                with open("error.log", "a", encoding="utf-8") as logf:
                    logf.write(
                        f"[ArchiveWorker] Extraction skipped: No disk images found "
                        f"in existing folder {self.output_path} at {datetime.now()}\n"
                    )
                print("[ArchiveWorker] Emitting error signal for no disk images found.")
                self.signals.error.emit(
                    f"Extraction skipped: No disk images found in existing folder "
                    f"{self.output_path}."
                )
                return
    ...
    def _compress(self):
        ...
        if not os.path.exists(self.input_path):
            with open("error.log", "a", encoding="utf-8") as logf:
                logf.write(
                    f"[ArchiveWorker] Input path not found: {self.input_path} at "
                    f"{datetime.now()}\n"
                )
        ...
        if not self.output_path:
            ...
            self.output_path = (
                f"{os.path.dirname(self.input_path)}/{base_name}.{self.archive_format}"
            )
        ...
        try:
            if self.archive_format == "zip":
                ...
                self.signals.error.emit(
                    f"Unsupported archive format: {self.archive_format}"
                )
                return
    ...
    def _extract_zip(self):
        ...
        try:
            ...
            with zipfile.ZipFile(self.input_path, "r") as zip_ref:
                ...
                for i, file_info in enumerate(file_list):
                    ...
                    try:
                        if self.password:
                            ...
                            zip_ref.extract(
                                file_info, self.output_path, pwd=self.password
                            )
                        else:
                        ...
                        if (
                            ...
                            self.signals.error.emit(
                                f"Password required or incorrect for file: "
                                f"{file_info.filename}"
                            )

                        else:
                            self.signals.error.emit(
                                f"Error extracting {file_info.filename}: {str(e)}"
                            )
                        return
                    except Exception as e:
                        self.signals.error.emit(
                ...
                self.signals.progress.emit(
                    100, f"Successfully extracted {file_count} files"
                )
    ...
    def _extract_7z(self):
        ...
        try:
            with py7zr.SevenZipFile(
                ...
                self.signals.progress.emit(
                    0, f"Extracting archive with {total_files} files..."
                )
                try:
                    z.extractall(path=self.output_path)
                except Exception as extract_exc:
                    self.signals.error.emit(
                        f"Error during 7z extraction: {str(extract_exc)}"
                    )
                    return
    ...
    def _extract_patool(self):
        ...
        patoolib.extract_archive(
            self.input_path, outdir=self.output_path, interactive=False
        )
    ...
    def _compress_patool(self):
        ...
        if os.path.isdir(self.input_path):
            ...
            self.signals.progress.emit(
                100, f"Compressed {os.path.basename(self.input_path)}"
            )
...
class ArchiveManager:
    ...
    def extract(
        ...
        Args:
            ...
            output_path: Path for the extracted files (default: same as archive
                without extension)
            password: Password for encrypted archives
