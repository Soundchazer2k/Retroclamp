...
from PySide6.QtWidgets import (
    ...
    QFileDialog, QMessageBox, QLabel, QProgressBar, QLineEdit,
    QGroupBox, QDialog, QDialogButtonBox, QTextEdit, QComboBox
)
...
from utils import show_info, show_error, show_warning
...
class BatchWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            logging.info(f"[BatchWorker] Launching: {self.operation} -i {self.file_path} -o {self.output_dir}")
...
class BatchTab(QWidget):
        ...
        try:
            ...
            if hasattr(self, 'resume_btn'):
                ...
                self.resume_btn.setToolTip(
                    f"Resume from checkpoint created on {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(latest_checkpoint.get('modified')))}"
                )
                
            # Show a status message in the UI
            if hasattr(self, 'status_label'):
                self.status_label.setText("Checkpoints available - click 'Resume from Last Checkpoint' to continue")
                
            # Log the checkpoint info
            logging.info(f"Found {len(checkpoints)} checkpoint(s). Latest: {latest_checkpoint.get('filename')}")
    ...
    def dropEvent(self, event: QDropEvent):
        ...
        for url in urls:
            ...
            if not file_path:
                logging.warning(f"Could not convert URL to local file path: {url.toString()}")
                continue
    ...
    def export_log(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(self, "Export Log", "retroclamp_batch_log.txt", "Text Files (*.txt);;HTML Files (*.html)")
        if filename:
            content = self.log_text.toHtml() if filename.endswith(".html") else self.log_text.toPlainText()
    ...
    def _process_dropped_directory(self, dir_path: str, output_dir: str):
        ...
        if not hasattr(self, 'operation_combo') or not self.operation_combo:
            ...
            logging.warning("operation_combo not initialized, defaulting to COMPRESS operation")
        else:
        ...
        if operation == CHDTaskType.COMPRESS:
            ...
            extensions = ['.chd']
        self.log_message(f"Scanning directory for supported files: {dir_path}", level="info")
        ...
        def on_extraction_finished(temp_dir, file_path):
            self.log_message(f"Extracted archive to temp directory: {temp_dir}", level="success")
            ...
            def on_scan_progress(found, scanned, current):
                self.log_message(f"Scanning extracted dir: {found} images found, {scanned} files checked...", level="info")
            def on_scan_error(msg):
                self.log_message(f"Disk image scan error in {temp_dir}: {msg}", level="error")
            def on_scan_finished(disk_images):
                nonlocal found_files
                if disk_images:
                    for extracted_path in disk_images:
                        self.log_message(f"Found supported image in extracted archive: {extracted_path}", level="success")
                        self._add_file_to_queue(extracted_path, output_dir)
                        found_files += 1
                else:
                    self.log_message(f"No supported files found in extracted archive: {file_path}", level="warning")
                ...
                if not pending_extractions and found_files == 0:
                    self.log_message(f"No supported files found in dropped directory: {dir_path}", level="warning")
                    if hasattr(self, 'update_status_indicator'):
                        self.update_status_indicator("No supported files found in the dropped folder(s).")
        ...
        for root, _, files in os.walk(dir_path):
            for file in files:
                ...
                if operation == CHDTaskType.COMPRESS and ext in archive_exts:
                    self.log_message(f"Found archive in directory: {file_path}", level="info")
                    ...
                    def make_on_finished(file_path):
                        def handle(success, msg, temp_dir):
                            if success:
                                ...
                                self.log_message(f"Extraction failed for {file_path}: {msg}", level="error")
                        return handle
                    ...
                    self.log_message(f"Found supported file in directory: {file_path}", level="success")
                    self._add_file_to_queue(file_path, output_dir)
                    found_files += 1
        if not pending_extractions and found_files == 0:
            self.log_message(f"No supported files found in dropped directory: {dir_path}", level="warning")
    ...
    def _add_archive_to_queue(self, archive_path: str, output_dir: str):
        ...
        logging.info(f"Adding archive to queue: {archive_path} with output dir {output_dir}")
        ...
        if hasattr(self, 'status_label') and self.status_label:
            self.status_label.setText(f"Added {os.path.basename(archive_path)} to queue")
        self.log_message(f"Added {os.path.basename(archive_path)} to queue", level="success")
    ...
    def setup_ui(self):
        ...
        self.log_text.setStyleSheet("QTextEdit { background: #181a20; color: #c8c8c8; font-family: Consolas, monospace; font-size: 12px; }")
        ...
        self.operation_combo.addItem("Extract Hard Disk from CHD", CHDTaskType.EXTRACT_HD)
        self.operation_combo.addItem("Extract Raw Data from CHD", CHDTaskType.EXTRACT_RAW)
        ...
        self.load_checkpoint_btn.setToolTip("Load a previously saved batch processing state")
    ...
    def save_checkpoint(self):
        """Save the current batch processing state to a checkpoint file using CheckpointManager."""
        if not self.files:
        ...
        try:
            ...
            metadata = {
                'operation': self.operation_combo.currentData(),
                'compression': self.compression_combo.currentData() if hasattr(self, 'compression_combo') else None,
    ...
    def load_checkpoint(self, checkpoint_file=None):
        ...
        Args:
            checkpoint_file: Path to the checkpoint file. If None, loads the latest checkpoint.
        ...
        try:
            ...
            show_info(f"Checkpoint loaded successfully: {os.path.basename(checkpoint_file)}")
            return True
    ...
    def update_status_indicator(self, status=None):
        ...
        if status == "processing":
            self.status_indicator.setText("Status: <span style='color:#50fa7b'>Processing</span>")
        elif status == "paused":
            self.status_indicator.setText("Status: <span style='color:#ffb86c'>Paused</span>")
        elif status == "error":
            self.status_indicator.setText("Status: <span style='color:#ff5555'>Error</span>")
        elif status == "completed":
            self.status_indicator.setText("Status: <span style='color:#50fa7b'>Completed</span>")
        else:
            self.status_indicator.setText("Status: <span style='color:#f1fa8c'>Idle</span>")
    ...
    def update_checkpoint_preview(self, checkpoint_path):
        ...
        """
        try:
            if not hasattr(self, 'checkpoint_manager') or not hasattr(self, 'checkpoint_info'):
                return False
            ...
            current_file = next((f for f in files if f.get('status') == 'in_progress'), files[0] if files else {})
            ...
            <body style='color:#f8f8f2;'>
                ...
                <p><b>Progress:</b> {completed} completed, {in_progress} in progress, {pending} pending</p>
                ...
                if hasattr(self, 'current_task_index') and hasattr(self, 'tasks'):
                    if self.current_task_index < len(self.tasks):
                        self.status_label.setText(
                            f"Batch processing aborted. {self.current_task_index}/{len(self.tasks)} files processed."
                        )
                    else:
                        self.status_label.setText(
                            f"Batch processing completed. {self.current_task_index}/{len(self.tasks)} files processed."
                        )
    ...
    def toggle_pause_resume(self):
        ...
        if self.is_paused:
            ...
            if hasattr(self, 'status_label') and hasattr(self, 'current_task_index') and hasattr(self, 'tasks'):
                self.status_label.setText(f"Processing paused. {self.current_task_index}/{len(self.tasks)} files processed.")
            if hasattr(self, 'show_busy_indicator'):
            ...
            # Update status and overlay if attributes exist
            if hasattr(self, 'status_label') and hasattr(self, 'current_task_index') and hasattr(self, 'tasks'):
                self.status_label.setText(f"Processing resumed. {self.current_task_index}/{len(self.tasks)} files processed.")
            if hasattr(self, 'hide_busy_indicator'):
                self.hide_busy_indicator()
            
            # Continue processing if not already processing
            if (not hasattr(self, 'is_processing') or not self.is_processing) and hasattr(self, 'process_next_task'):
                self.process_next_task()
    ...
    def on_task_finished(self, success: bool, message: str, row: int):
        ...
        if hasattr(self, 'progress_bars') and row < len(self.progress_bars):
            style = ""
            if success:
                style = "QProgressBar::chunk { background-color: #4CAF50; }"  # Green for success
                self.progress_bars[row].setValue(100)  # Ensure it shows as complete
            else:
                style = "QProgressBar::chunk { background-color: #F44336; }"  # Red for failure
                self.progress_bars[row].setValue(100)  # Ensure it shows as complete
    ...
    def on_task_error(self, error_message: str, row: int):
        ...
        if hasattr(self, 'progress_bars') and row < len(self.progress_bars):
            style = "QProgressBar::chunk { background-color: #F44336; }"  # Red for error
            self.progress_bars[row].setStyleSheet(style)
    ...
    def check_archive_compatibility(self, archive_path: str, row: int) -> bool:
        ...
        """
        self.log(row, f"Checking archive compatibility: {os.path.basename(archive_path)}")
    ...
    def extract_archive(self, archive_path, row):
        ...
        try:
            ...
            self.archive_manager.extract_archive(
                archive_path,
                temp_dir,
                progress_callback=lambda value, msg: self.on_task_progress(value, msg, row),
                error_callback=lambda msg: self.on_task_error(msg, row),
                finished_callback=lambda success, msg: self.on_task_finished(success, msg, row)
            )
                        
            # Find files matching the target extension (This logic might be better in on_task_finished)
            extracted_files = []
            ...
            if extracted_files:
                logging.info(f"Found {len(extracted_files)} compatible files in archive in {temp_dir}")
                self.log(row, f"Found {len(extracted_files)} compatible files in archive")
            
            # The actual extraction is asynchronous via archive_manager, so we don't return here.
            # The on_task_finished callback will handle the next steps.
            # We return the temp_dir path if created successfully.
            return temp_dir
            
        except Exception as e:
            error_msg = f"Error extracting archive {os.path.basename(archive_path)}: {e}"
    ...
    def start_processing(self):
        ...
        for file_path in self.files:
            ...
            output_path = os.path.join(output_dir, base_name + ".chd" if self.operation_combo.currentData() == CHDTaskType.COMPRESS else ".bin")
        ...
        for idx, signals in enumerate(signals_list):
            ...
            signals.progress.connect(lambda val, msg, i=idx: self.on_task_progress(val, msg, i))
            signals.finished.connect(lambda success, msg, i=idx: self.on_task_finished(success, msg, i))
    ...
    def cleanup_temp_directories(self, temp_dirs=None):
        ...
        Args:
            temp_dirs: List of temporary directories to clean up. If None, uses self.temp_directories
        """
    ...
    def process_next_task(self):
        ...
        if self.current_task_index < len(self.files):
            ...
            # Update status
            self.status_label.setText(f"Processing {self.current_task_index + 1} of {self.total_files}: {file_name}")
    ...
    def process_archive_file(self, file_path):
        ...
        if not self.check_archive_compatibility(file_path, row):
            ...
            self.task_completed(success=False, message=f"No compatible files in {file_name}")
            return
    ...
    def process_regular_file(self, file_path: str):
        ...
        try:
            ...
            self.worker.error.connect(
                lambda msg, r=row: self.on_task_error(msg, r))
            self.worker.finished.connect(
                lambda: self._on_worker_finished(row))
    ...
    def all_tasks_completed(self):
        ...
        try:
            ...
            self._show_completion_message(f"Error completing tasks: {str(e)}", is_error=True)
    ...
    def _update_ui_after_completion(self):
        ...
        try:
            ...
            message = f"Processing complete. {self.processed_files} files processed successfully."
    ...
    def task_completed(self, success=True, message=None):
        ...
        try:
            ...
            progress = int((self.processed_files + self.failed_files) / total * 100) if total > 0 else 0
            ...
            # Update status label
            status_message = f"Processed: {self.processed_files} | Failed: {self.failed_files} | Total: {total}"
            if hasattr(self, 'status_label'):
            ...
            if not self.is_aborting and hasattr(self, 'files') and (self.processed_files + self.failed_files) < len(self.files):
    ...
    # Removed duplicate task-related function definitions - keeping only one version of each function
