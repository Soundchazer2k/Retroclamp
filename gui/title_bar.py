...
class TitleBar(QWidget):
    ...
    def _update_stylesheet(self):
        ...
        # Create a slightly lighter version of bg_color for gradient effect (like Windows title bar)
        bg_color_rgb = get_color_from_hex(bg_color)
        lighter_bg = f"rgb({min(bg_color_rgb[0]+15, 255)}, {min(bg_color_rgb[1]+15, 255)}, {min(bg_color_rgb[2]+15, 255)})" 
        
        self.setStyleSheet(f"""
            QWidget {{ 
                background-color: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 {lighter_bg}, stop:1 {bg_color}); 
                color: {text_color};
                border: none;
            }}
            QPushButton {{ 
