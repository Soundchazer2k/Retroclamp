...
from modules.ui_functions import load_svg_icon, apply_theme
...
class ThemeTab(QWidget):
    ...
    def setup_ui(self):
        ...
        # Description
        desc_label = QLabel(
            "Customize the application theme by selecting a predefined theme or creating your own. "
        ...
        self.color_buttons["secondaryBackground"] = ColorButton("secondaryBackground", "#44475a")
        bg_layout.addRow("Secondary Background:", self.color_buttons["secondaryBackground"])
        
        # Tertiary background color
        self.color_buttons["tertiaryBackground"] = ColorButton("tertiaryBackground", "#6272a4")
        bg_layout.addRow("Tertiary Background:", self.color_buttons["tertiaryBackground"])
    ...
    def load_themes(self):
        ...
        for filename in os.listdir(themes_dir):
            if filename.startswith("theme_") and filename.endswith(".json") and filename != "theme.json" and filename != "theme_light.json":
                # Get theme name
    ...
    def check_accessibility(self):
        ...
        if not is_accessible(ratio):
            issues.append(f"Text on background contrast ratio is {ratio:.2f}:1 (should be at least 4.5:1)")
        
        # Text on secondary background
        secondary_bg = colors.get("secondaryBackground", "#44475a")
        ratio = calculate_contrast_ratio(text, secondary_bg)
        if not is_accessible(ratio):
            issues.append(f"Text on secondary background contrast ratio is {ratio:.2f}:1 (should be at least 4.5:1)")
        ...
        if not is_accessible(ratio):
            issues.append(f"Text on tertiary background contrast ratio is {ratio:.2f}:1 (should be at least 4.5:1)")
        
        # Primary on background
        primary = colors.get("primary", "#bd93f9")
        ratio = calculate_contrast_ratio(primary, background)
        if not is_accessible(ratio):
            issues.append(f"Primary on background contrast ratio is {ratio:.2f}:1 (should be at least 4.5:1)")
        
        # Update accessibility label
        if issues:
            self.accessibility_label.setText("Accessibility issues detected:\n\n" + "\n".join(issues))
    ...
    def reset_theme(self):
        ...
        if theme_name == "dracula":
            ...
            QMessageBox.information(
                self,
                "Reset Theme",
                "Custom themes cannot be reset to default."
            )
            return
    ...
    def save_theme(self):
        ...
        if is_builtin:
            ...
            new_name, ok = QFileDialog.getSaveFileName(
                self,
                "Save Theme As",
                os.path.join(os.path.dirname(os.path.dirname(__file__)), "config", "theme_custom.json"),
                "Theme Files (*.json)"
            )
        ...
        QMessageBox.information(
            self,
            "Theme Saved",
            f"Theme '{theme_name}' has been saved successfully."
        )
