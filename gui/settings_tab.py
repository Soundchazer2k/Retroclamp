...
class SettingsTab(QWidget):
    ...
    def load_settings(self):
        ...
        self.remember_pos_check.setChecked(
            self.settings.get("general", "remember_position", True)
        )
        ...
        self.output_dir_edit.setText(
            self.settings.get("files", "default_output_dir", "")
        )
        self.file_filters_edit.setText(
            self.settings.get(
                "files", "default_filters", "*.iso;*.bin;*.img;*.cue;*.zip;*.7z;*.rar"
            )
        )

        # Logging
        self.enable_logging_check.setChecked(
            self.settings.get("logging", "enabled", True)
        )
        self.log_level_combo.setCurrentText(
            self.settings.get("logging", "level", "Info")
        )
        self.log_file_edit.setText(
            self.settings.get("logging", "file", "retroclamp.log")
        )
        ...
        self.compression_level_combo.setCurrentText(
            self.settings.get("chdman", "compression_level", "Normal")
        )
        self.hunk_size_combo.setCurrentText(
            self.settings.get("chdman", "hunk_size", "16 KB")
        )
        ...
        # Performance
        self.enable_threading_check.setChecked(
            self.settings.get("performance", "multithreading", True)
        )
        ...
        self.auto_start_check.setChecked(
            self.settings.get("performance", "auto_start", True)
        )
        self.buffer_size_spin.setValue(
            self.settings.get("performance", "buffer_size", 8)
        )
        self.limit_memory_check.setChecked(
            self.settings.get("performance", "limit_memory", False)
        )
        self.max_memory_spin.setValue(
            self.settings.get("performance", "max_memory", 1024)
        )
    ...
    def detect_chdman(self):
        ...
        if chdman_path and os.path.exists(chdman_path):
            ...
            QMessageBox.information(
                self, "CHDMAN Detected", f"CHDMAN executable found at:\n{chdman_path}"
            )
        else:
            QMessageBox.warning(
                self,
    ...
    def reset_settings(self):
        ...
        QMessageBox.information(
            self, "Settings Reset", "All settings have been reset to default."
        )
    ...
    def save_settings(self):
        ...
        # General settings
        self.settings.set(
            "general", "confirm_exit", self.confirm_exit_check.isChecked()
        )
        self.settings.set(
            "general", "remember_position", self.remember_pos_check.isChecked()
        )
        self.settings.set(
            "general", "check_updates", self.check_updates_check.isChecked()
        )
        ...
        self.settings.set(
            "chdman", "compression_level", self.compression_level_combo.currentText()
        )
        ...
        # Performance
        self.settings.set(
            "performance", "multithreading", self.enable_threading_check.isChecked()
        )
        ...
        self.settings.set(
            "performance", "auto_start", self.auto_start_check.isChecked()
        )
        self.settings.set("performance", "buffer_size", self.buffer_size_spin.value())
        self.settings.set(
            "performance", "limit_memory", self.limit_memory_check.isChecked()
        )
