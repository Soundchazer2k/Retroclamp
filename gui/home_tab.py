...
class HomeTab(QWidget):
    ...
    def setup_ui(self):
        ...
        logo_label.setPixmap(load_svg_icon("device-gamepad-2", 64, "#bd93f9").pixmap(64, 64))
        ...
        # Description
        desc_label = QLabel(
            "RetroClamp is a modern GUI for CHDMAN operations, allowing you to compress, "
            "decompress, and manage disk images with ease. Select an option below to get started."
        )
        ...
        # We don't need to add features_layout here as it already has a parent (features_widget)
    ...
    def connect_signals(self):
        ...
        self.compress_btn.enterEvent = lambda e: self.update_description(
            "Compress Disk Images",
            "Convert disk images (ISO, BIN, IMG, etc.) to CHD format for efficient storage "
            "and emulation compatibility. CHD files are smaller and maintain all the original data."
        )
        
        self.extract_btn.enterEvent = lambda e: self.update_description(
            "Extract CHD Files",
            "Extract disk images from CHD files to various formats such as ISO, BIN, or AVI. "
            "Choose the output format that best suits your needs."
        )
        
        self.tools_btn.enterEvent = lambda e: self.update_description(
            "Additional Tools",
            "Access additional tools for working with disk images, including SCUMMVM configuration "
            "generation, metadata extraction, and more."
        )
