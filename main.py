...
from modules.ui_functions import load_svg_icon, apply_theme, resize_grips
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Remove all window decoration flags
        flags &= ~Qt.WindowType.WindowTitleHint
        flags &= ~Qt.WindowType.WindowSystemMenuHint
        # Turn on frameless but keep resizing capability
        flags |= Qt.WindowType.FramelessWindowHint
        # Add back only the specific flags we need
        flags |= (
    ...
    def setup_ui(self):
        ...
        # Main layout - using vertical layout to include title bar
        ...
        # Main content layout (for sidebar and content area)
        ...
        # collapsed width → increase tap target
        ...
        # We already have a title bar in the main layout, so we don't need another one here
        ...
        text_color = colors.get("text", "#f8f8f2")  # Use text color from theme (same as other icons)
        ...
        self.resize_handle.setStyleSheet("#resizeHandle { background-color: transparent; border: none; }"
    ...
    def connect_home_signals(self):
        ...
        self.home_page.compress_clicked.connect(lambda: self.change_page(self.compression_page))
        self.home_page.extract_clicked.connect(lambda: self.change_page(self.extraction_page))
    ...
    def connect_signals(self):
        ...
        self.btn_compress.clicked.connect(lambda: self.change_page(self.compression_page))
    ...
    def change_page(self, page):
        ...
        for button in buttons:
            ...
            if collapsed:
                ...
                button.setIconSize(QSize(48, 48))  # Make icons even larger in collapsed mode
            else:
    ...
    def toggle_menu(self):
        ...
        if target_width == min_width:
            ...
            for btn in (self.btn_home, self.btn_compress, self.btn_extract, 
                       self.btn_tools, self.btn_theme, self.btn_settings, self.toggle_button):
            ...
            for btn in (self.btn_home, self.btn_compress, self.btn_extract, 
                       self.btn_tools, self.btn_theme, self.btn_settings, self.toggle_button):
                btn.setStyleSheet("padding-left: 16px; text-align: left;")  # Fixed positioning
    ...
    def showEvent(self, event):
        ...
        # Position the resize handle at the bottom-right corner when window is first shown
    ...
    def _update_resize_handle_position(self):
        ...
        QTimer.singleShot(0, lambda: self.resize_handle.move(
            self.content.width() - 16, 
            self.content.height() - 16
        ))
    ...
    def _update_nav_button_states(self):
        ...
        for btn, label in self.nav_buttons:
            if collapsed:
                # hide text, bump icon
                btn.setText("")
                btn.setIconSize(QSize(48, 48))  # Increased from 40 to 48 for larger icons
                btn.setStyleSheet("text-align: center;")  # center align when collapsed
            else:
                # restore
                btn.setText(label)
                btn.setIconSize(QSize(32, 32))  # standard size with text
                btn.setStyleSheet("text-align: left; padding-left: 16px;")  # left align when expanded
    ...
    def eventFilter(self, obj, event):
        ...
        if obj == self.resize_handle:
            if event.type() == event.Type.MouseButtonPress:
                ...
                self._resize_start_pos = QPoint(event.globalPosition().x(), event.globalPosition().y())
                self._resize_start_size = self.size()
                return True
            elif event.type() == event.Type.MouseMove and hasattr(self, '_resize_start_pos'):
                # Calculate the new size
                current_pos = QPoint(event.globalPosition().x(), event.globalPosition().y())
                delta = current_pos - self._resize_start_pos
                new_size = self._resize_start_size + QSize(delta.x(), delta.y())
                # Apply minimum size constraints
                new_size = QSize(max(new_size.width(), 800), max(new_size.height(), 600))
                # Resize the window
