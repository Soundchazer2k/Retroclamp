...
class AppSettings:
    ...
    def reset(self, category: Optional[str] = None, key: Optional[str] = None) -> None:
        ...
        if category is None:
            ...
            if key is None:
                ...
                self.settings[category] = self._deep_copy_dict(
                    self.DEFAULT_SETTINGS[category]
                )
                    ...
                    for key, value in category_settings.items():
                        if key in self.settings[category]:
                            ...
                            if isinstance(default_value, bool) and not isinstance(
                                ...
                                value = int(value)
                            elif isinstance(default_value, float) and not isinstance(
                                value, float
                            ):
                                value = float(value)
    ...
    def _deep_copy_dict(d: Dict[str, Any]) -> Dict[str, Any]:
        """Create a deep copy of a dictionary, recursively handling lists and dicts."""
        result: Dict[str, Any] = {}
        for key, value in d.items():
            if isinstance(value, dict):
                result[key] = AppSettings._deep_copy_dict(value)
            elif isinstance(value, list):
                result[key] = AppSettings._deep_copy_list(value)
            else:
                result[key] = value
        return result
    ...
    def _deep_copy_list(lst: List[Any]) -> List[Any]:
        """Create a deep copy of a list, recursively handling dicts and sublists."""
        return [
            (
                AppSettings._deep_copy_dict(item)
                if isinstance(item, dict)
                else (
                    AppSettings._deep_copy_list(item)
